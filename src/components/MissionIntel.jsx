...
export default function MissionIntel() {
  ...
  return (
    <motion.aside
      ...
      style={{ boxShadow: '0 0 42px rgba(245,158,11,0.24), 0 0 84px rgba(217,70,239,0.16), inset 0 0 28px rgba(157,23,77,0.16)' }}
    >
      ...
      <div className="flex items-center justify-between">
        <span className="hud-label text-aurora">DAIMYO COMMAND</span>
      ...
      <div className="mt-4 rounded-2xl border border-cyan/25 bg-ink-900/70 p-3">
        <div className="flex items-center justify-between">
          ...
          <span className="hud-label text-cyan">{completion}% MASTERED</span>
        </div>
      ...
      <ul className="mt-3 grid gap-2">
        {protocol.tasks.map((task) => (
          <li key={task} className="flex items-center justify-between rounded-xl border border-cyan/20 bg-ink-900/55 px-3 py-2">
            <span className="hud-label text-[9px] text-cyan/70">RITE</span>
      ...
      <div className="mt-3 rounded-xl border border-cyan/20 bg-ink-900/55 p-3">
        <div className="hud-label opacity-70">ACTIVE ARSENAL</div>
